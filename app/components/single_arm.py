...
def generate_cli_code_single_arm_continuous(params):
    """
    Generate reproducible CLI code for single-arm continuous outcomes with algorithm transparency.
    """
    ...
    # Get the source code of the key algorithm based on calculation type
    if calculation_type == 'Sample Size':
        key_function = one_sample_t_test_sample_size
        algorithm_source = get_function_source(key_function)
        function_name = "one_sample_t_test_sample_size"
    else:  # Power calculation
        key_function = one_sample_t_test_power
        algorithm_source = get_function_source(key_function)
        function_name = "one_sample_t_test_power"
    
    script = f'''#!/usr/bin/env python3
"""
Enhanced Single-Arm Trial Analysis - Continuous Outcome ({calculation_type})
Generated by DesignPower CLI with Algorithm Transparency

This script performs {calculation_type.lower()} calculation for a single-arm trial
with continuous outcomes. The complete source code of the statistical algorithm
is included below for full transparency and educational purposes.
"""
...
def main():
    ...
    # Study parameters (from your input)
    mean = {mean}  # Expected sample mean
...
'''
    ...
    if calculation_type == 'Sample Size':
        script += f'''    
    # Calculate sample size using the algorithm shown above
    ...
    print("Source: core.designs.single_arm.continuous")
    print("Method: One-sample t-test with exact degrees of freedom")
    print("Algorithm source code: See complete implementation above")
'''
    ...
    # Calculate power using the algorithm shown above
